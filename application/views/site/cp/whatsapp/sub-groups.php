<ul class="page-breadcrumb breadcrumb">
	...
	<li>
		<a href="<?=$this->SITE_URL?>cp/whatsapp/groups"><?=lang("main_groups")?></a>
		<i class="fa fa-angle-left"></i>
	</li>
	<li>
		<a href="<?=$this->URI?>"><?=lang("sub_groups")?></a>
	</li>
</ul>
<div class="col-md-8">
<form action="" method="post">
	...
	<div class="portlet box blue">
		<div class="portlet-title">
			<div class="caption">
				<i class="fa fa-group"></i>المجموعات الفرعية
			</div>
			<div class="tools">
			</div>
		</div>
...
<div class="table-responsive">
	...
	<tbody>
		...
		<tr>
			<td><input class="check_input" type="checkbox" name="checks[]" value="<?=$group->id?>"/></td>
			<td style="width:20%;"><div id="group_title_<?=$group->id?>"><?=$group->title?></div></td>
			<td style="width:20%;"><?=$group->count?></td>
			<td style="width:20%;"><?=date("d-m-Y",$group->create_time)?></td>
			<td>
				<?php // if($permissions->users["users_modify"]=="1"){?>
				<a href="<?=$this->SITE_URL?>cp/whatsapp/groups/GroupNumbers/<?=$group->id?>" class="btn default btn-xs dark tooltips" title="<?=lang("show_group_numbers")?>"><b class="fa fa-file-o"></b></a>
...
<div class="col-md-4">
	<ul class="pagination pagination-sm">
		<li><a href="<?=($page_num>0?$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?search_word=$search&limit=".$limit."&page=".($page_num-1):"#")?>"><?=lang("prev")?></a></li>
			<?php for($i=0;$i<ceil($groups["count"]/$limit);$i++){?>
			<li class="<?=($page_num==$i?"active":"")?>"><a href="<?=($page_num!=$i?$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?limit=$limit&page=$i&search_word=$search":"#")?>"><?= $i + 1?></a></li>
			<?php }?>
			<li><a href="<?=$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?search_word=$search&limit=".$limit."&page=".(int)($page_num+1)?>"><?=lang("next")?></a></li>
	</ul>
	</div>
	<?php }?>
	</div>
...
<div class="portlet box yellow tabbable">
	...
	<div class="well">
		<?=lang("show")?>:
		<a href="<?=$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?limit=25&page=$page_num&search_word=$search"?>"><span class="label label-info">25</span></a>
		<a href="<?=$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?limit=50&page=$page_num&search_word=$search"?>"><span class="label label-success">50</span></a>
		<a href="<?=$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?limit=100&page=$page_num&search_word=$search"?>"><span class="label label-warning">100</span></a>
		<a href="<?=$this->SITE_URL."cp/whatsapp/groups/subGroups/$group_id?limit=all&page=0&search_word=$search"?>"><span class="label label-important"><?=lang("all")?></span></a>
	</div>
	<?php }?>
