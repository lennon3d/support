...
class WhatsProt
{
    ...
    const MEDIA_FOLDER = './assets/uploads';                           // The relative folder to store received media files
    const PICTURES_FOLDER = './assets/uploads';                     // The relative folder to store picture files
    ...
    public function codeRegister($code)
    {
        ...
        /**
     *	added by Marwan Zakariya <marwanzk0@gmail.com>
     *
     * check if number wether have whatsapp registered or number
     *
     * @param string $number
     *   can consist + or 00 or without.
     *
     * @return boolean
	 *
     */
    ...
    public function checkNumber ($number)
    {
    	...
    	$numsync = new WhatsAppContactSync ($this->phoneNumber,  $this->password, $number);
    	$was = $numsync->executeSync ();
    	return $was;
    }
    ...
    public function codeRequest($method = 'sms', $countryCode = null, $langCode = null)
    {
        ...
        if ($response->status == 'ok') {
            ...
            if (isset($response->reason) && $response->reason == "too_recent") {
                ...
                $this->eventManager()->fire('onCodeRequestFailed', array($this->phoneNumber, $method, $response->reason));
                //throw new Exception('There was a problem trying to request the code.');
            }
        } else {
     ...
     */
    public function getMessages($add=array())
    {
     ...
     */
    public function pollMessages($add=array())
    {
        $this->processInboundData($this->readData(), $add);
    }
     ...
     *   The text message.
     */
    public function sendMessage($to, $txt, $add=array())
    {
        $txt = $this->parseMessageForEmojis($txt);
        $bodyNode = new ProtocolNode("body", null, null, $txt);
        $this->sendMessageNode($to, $bodyNode, $add);
    }
     ...
     */
    public function sendStatusUpdate($txt, $add=array())
    {
        $bodyNode = new ProtocolNode("body", null, null, $txt);
        ...
        $this->sendNode($messageNode);
        $this->eventManager()->fire('onSendStatusUpdate', array($this->phoneNumber, $txt)+$add);
        //listen for response
        $this->pollMessages();
    }
...
//                finfo_close($finfo);
    ...
    /**
     * Get a decoded JSON response from Whatsapp server multi request threads
     *
     * @param  string $host  The host URL
     * @param  array $query A associative multi dimensional array of keys and values to send to server.
     * @return object   NULL is returned if the json cannot be decoded or if the encoded data is deeper than the recursion limit
     */
    protected function getMultiResponse($host, $query)
    {
    	$requests = array();
        // Build the url.
        $url = $host . '?';

        foreach ($query as $id => $request) {
        	foreach($request as $key => $value)
            	$requests[$i] = $url . $key . '=' . $value . '&';
        	$requests[$i] = rtrim($requests[$i], '&');
        }
        ...
        {
        	$curl_array[$i] = curl_init($request);
        	curl_setopt($curl_array[$i], CURLOPT_RETURNTRANSFER, true);
        	curl_setopt($curl_array[$i], CURLOPT_HEADER, 0);
        	curl_setopt($curl_array[$i], CURLOPT_USERAGENT, static::WHATSAPP_USER_AGENT);
        	curl_setopt($curl_array[$i], CURLOPT_HTTPHEADER, array('Accept: text/json'));
        ...
        foreach($requests as $i => $request)
        {
        	$res[$request] = curl_multi_getcontent($curl_array[$i]);
        }
     ...
     *   The data to process.
     */
    protected function processInboundData($data, $add=array())
    {
        try {
            ...
            while ($node != null) {
                ...
                if ($node->getTag() == "challenge") {
                    ...
                    file_put_contents($dir_path."nextChallenge.dat", $challengeData);
                    $this->writer->setKey($this->outputKey);
...
//            $height = $json->height;
     ...
     */
    protected function sendMessageNode($to, $node, $add=array())
    {
        $serverNode = new ProtocolNode("server", null, null, "");
        $xHash = array();
        $xHash["xmlns"] = "jabber:x:event";
